...
Sinh viên: Chế độ chỉ xem, tìm kiếm và sắp xếp (các nút thêm,sửa,xóa bị ẩn đi với SV).
Tìm kiếm thông minh **Fuzzy Search**: Sử dụng thuật toán tìm kiếm gần đúng, cho phép tìm thấy kết quả ngay cả khi gõ thiếu hoặc sai.
Đặc biệt: Tên sinh viên khớp với từ khóa sẽ được highlight màu xanh lục.
**Sắp xếp**: Bao gồm sắp xếp theo ID, Điểm và Tên.
